...
class SM_Test(unittest.TestCase):
    ...
    # def test_upload_weekExam(self,upload_url, upload_data, expect):
    #     data = {'phase':upload_data['phase'],'week':upload_data['week'],'class_id':upload_data['class_id']}
    #     path = {"token":open(upload_data['token'],'rb')}
    #     res = SM_Action(self.session).do_upload(upload_url, data,path)
    #     if '上传成功'in res.text:
    #         actual = 'pass'
    #     else:
    #         actual = 'fail'
    ...
    def test_upload_examination(self, upload_url, upload_data, expect):
        data = {'phase':upload_data['phase'],'class_id':upload_data['class_id']}
        path = {"token":open(upload_data['token'],'rb')}
        res = SM_Action(self.session).do_upload(upload_url, data,path)
        if '上传成功'in res.text:
            actual = 'pass'
